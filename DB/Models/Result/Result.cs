...
public class NotFoundResult<T> : Result<T>, IErrorResult
{
    public NotFoundResult(string message) : this(message, Array.Empty<Error>())
    {

    }

    public NotFoundResult(string message, IReadOnlyCollection<Error> errors) : base(default)
    {
        Message = message;
        Success = false;
        Errors = errors ?? Array.Empty<Error>();
    ...
    public static implicit operator ErrorResult(NotFoundResult<T> errorResult)
    {
        return new ErrorResult(errorResult.Message, errorResult.Errors);
    }

    public virtual NotFoundResult<TType> ToGeneric<TType>()
    {
        return new NotFoundResult<TType>(Message, Errors);
    }
}
